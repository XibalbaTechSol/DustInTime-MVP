...
import type { Booking, Point, Maneuver } from '../types';
...
const CleanerNavigationPage: React.FC<CleanerNavigationPageProps> = ({ booking, onBack, onUpdateStatus }) => {
    ...
    const [view, setView] = useState<'map' | 'instructions'>('map');
    ...
    useEffect(() => {
        const fetchRoute = async () => {
            const { route, maneuvers } = await getRoute(startPos, endPos);
            setRoute(route);
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden animate-fade-in bg-slate-100">
            {view === 'map' ? (
                ...
                <div className="w-full h-full overflow-y-auto bg-white p-4 pt-20">
                    <h2 className="text-2xl font-bold mb-4">Turn-by-turn Instructions</h2>
                    <ul className="divide-y divide-slate-200">
                        {maneuvers.map((maneuver, index) => (
                            <li key={index} className="py-4 flex items-center">
                                ...
                                <div>
                                    <p className="text-lg font-semibold text-slate-800">{maneuver.instruction}</p>
                                    <p className="text-sm text-slate-500">{maneuver.length} miles, approx. {Math.round(maneuver.time / 60)} min</p>
                                </div>
                            </li>
                        ))}
                    </ul>
            ...
            <div className="absolute top-0 left-0 w-full p-4 z-[1000]">
                 <div className="container mx-auto flex justify-between items-center">
                    ...
                    <button
                        onClick={() => setView(view === 'map' ? 'instructions' : 'map')}
                        className="flex items-center space-x-2 bg-white/80 backdrop-blur-sm rounded-full px-4 py-2 shadow-lg hover:bg-white transition text-slate-800 font-semibold"
                        aria-label="Toggle view"
                    >
                        <span>{view === 'map' ? 'Instructions' : 'Map'}</span>
                    </button>
                </div>
            </div>
