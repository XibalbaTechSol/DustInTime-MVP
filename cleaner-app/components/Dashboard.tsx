 ...
 */
const Dashboard: React.FC<DashboardProps> = ({ user, onNavigate, onClose, bookings, onLogout }) => {
  ...
  const CleanerDashboard = () => {
      ...
      const myBookings = bookings.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        <div className="flex-shrink-0 flex items-center justify-between p-4 bg-base-200 dark:bg-neutral-focus border-b border-base-300 dark:border-slate-700">
            <h1 className="text-2xl font-bold text-neutral dark:text-base-200">My Dashboard</h1>
             <div className="flex items-center space-x-4">
                ...
                <button
                    onClick={onLogout}
                    className="flex items-center space-x-2 text-sm font-semibold text-error hover:text-error-focus transition-colors"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
